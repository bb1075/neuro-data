...
file = open('test4.txt')
# field[0] is SDAN number, field[1] is MRN, the reamining fileds list MR session number
for line in file:
...
# check if subject exists in project, if not create subject
        querysubj = createsubj(xnatID,destproject)
        print(querysubj)
#    for exp in expID:
        r = connect.put(f"{xnaturl}{querysubj}")
        if r.status_code == 200:
            print("worked - created subject",sdanID,"/",xnatID,"in project",destproject)
            print("worked")
        else :
            print("failed")
            print("failed - check subject information for",sdanID,xnatID,"/",expID)	

# for moving on MR session
...
#    for exp in expID:
        r = connect.put(f"{xnaturl}{queryexp}")
        if r.status_code == 200:
            print("worked - moved from project",xnatID,"/",expID,"to",destproject)
            print("worked")
        else :
            print("failed")
            print("failed - check subject information for",sdanID,xnatID,"/",expID)	
...
#https://fmrif-xnat.nimh.nih.gov/data/projects/EDB-NIMH/experiments/?format=json&columns=xnat:subjectData/label,xnat:mrSessionData/label,xnat:mrSessionData/ID,xnat:mrSessionData/date,xnat:mrSessionData/project,URI,xnat:mrSessionData/UID
